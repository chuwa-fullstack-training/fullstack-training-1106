...
const {authenticateToken, router}  = require('./middleware/auth')
...
app.get('/api/employees', async (req,res) =>{
    try{
        let queryEmployees = [];
        if(!req.user){
            queryEmployees = await Employee.find().select(' -_id firstName lastName');
        }else{
            const userEmployee = await Employee.findOne({firstName: {$eq: req.user.firstName}, lastName: {$eq: req.user.lastName}}).select('company');
            if(!userEmployee || !userEmployee.company){
                res.status(404).send("Employees Not Found");
                return;
            }
            const userCompany = userEmployee.company;
            console.log("test userCompany",userCompany);
            queryEmployees = await Employee.find({company: {$eq: userCompany}});
        } 
        if(queryEmployees){
