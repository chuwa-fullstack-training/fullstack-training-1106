...
const getAllEmployees = async (req, res) => {
    ...
    query = req.user ? query : query.select('firstName lastName -_id')
    const employees = await query.catch(err => res.status(500).json({message: 'Error finding all Employees', err}));
    res.status(200).json(employees);
}

const getEmployee = async (req, res) => {
    let query = await Employee.findById(req.params?.id)
    query = req.user ? query : query.select('firstName lastName -_id')
    const employee = await query.catch(err => res.status(500).json({message: 'Error finding all Employees', err}));
