...
const JWT_SECRET = 'read_from_file';
...
app.post('/login', async (req, res) => {
    ...
    if (!user || user.lastName !== password) { // use lastName as password @TODO
        res.status(401).json({ message: 'Invalid username or password' });
        return;
    }

    const token = jwt.sign({ userId: user._id, username: user.firstName }, JWT_SECRET, {
...
app.get('/api/employees', authenticateToken, async (req, res) => {
    ...
    if (req.user) {
        ...
        res.json(employees.map(({firstName, lastName}) => {firstName, lastName}));
    }
});
...
app.get('/api/companies/:companyId/employees', authenticateToken, async (req, res) => {
    const companyId = req.params.companyId;
    const employees = await Employee.find({ companyId });
    res.json(employees.filter(employee => employee.companyId == req.user.companyId));
});
