...
const server = http.createServer((req, res) => {
  ...
  if (method === 'GET') {
    if (url === '/') {
        ...
        // get the query params (such as "?title=a&content=b")
        ...
        fs.readFile(path.join(__dirname, 'home.html'), (err, html) => {
            ...
            res.write(html);
            // write the query string in the html as well
            if (queryParams) {
                res.write('<p>' + queryParams + '</p>');
            }
            res.end();
            }
        });
    ...
    if (url === '/create-post') {
        ...
        req.on('end', () => {
            ...
            // put the data of the submitted form in the query string of the url, 
            // before res.end() in POST method, redirect to the home.html page with the query string
            res.statusCode = 302;
